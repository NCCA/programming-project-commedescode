...
#include <algorithm>
flock::flock(size_t _num, size_t _maxAlive, int _NumOfBirb, ngl::Vec3 _pos) :
m_maxbirbs{_num}, m_maxAlive{_maxAlive}, m_NumOfBirb{_NumOfBirb}, m_pos{_pos}
{
...
#pragma omp parallel for
    for(size_t i=0; i<m_maxbirbs; ++i)
    {
        ...
        m_ppos[i] += ngl::Vec4(m_pdir[i] * m_speed * _dt, 0.0f);
...
void flock::resetbirb(size_t _i)
{
    ...
    m_pdir[_i] = emitDir * 0.1f + randomVectorOnCone() * 0.5f;
...
void flock::setSpeed(float speed) {
    m_speed = speed;
}

void flock::setMaxBirds(int maxBirds) {
    m_maxBirds = maxBirds;
}

void flock::setNumPerFrame(int numPerFrame) {
